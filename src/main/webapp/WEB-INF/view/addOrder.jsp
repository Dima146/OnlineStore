...
<html>
<head>
  <title>Add order | Online store</title>
    ...
    <div class="col-6 my-sm-3">
      ...
      <c:if test="${errorMessage == 'true'}">
        ...
        <form action="${pageContext.request.contextPath}/OnlineStore?command=confirmOrder" method="post">
          ...
          <div class="mb-3">
            <label for="address" class="form-label"><fmt:message bundle="${loc}" key="language.address"/></label>
            <input type="text" class="form-control" name="address" id="address" placeholder="City, Street, **" required>
          </div>
          <div class="mb-3">
            <label for="delivery-date" class="form-label"><fmt:message bundle="${loc}" key="language.deliveryDate"/></label>
          ...
          <h3 class="text-primary"><fmt:message bundle="${loc}" key="language.toPay"/>:
            <c:out value="${totalCost}"/> <fmt:message bundle="${info}" key="information.price"/></h3>
          ...
          <div class="row mb-3">
            <div class="col-sm mb-3">
              ...
              <div class="input-group w-100">
                <input type="text" class="form-control" id="card-number" name="card-number" placeholder="0101010101010101" pattern="^[0-9]{16}$" required></div>
            </div>
            <div class="col-sm">
              <label for="cvv" class="form-label">CVV</label>
              <div class="input-group">
              <input type="password" class="form-control" id="cvv" name="cvv" placeholder="000" pattern="^[0-9]{3}$" required></div>
            </div>
          </div>
          ...
          <div class="mb-3">
            ...
            <div class="input-group">
              <input type="text" id="cardholder-name" name="cardholder-name" class="form-control" placeholder="Name Surname" required maxlength="45" pattern="\b[A-ZА-Я].*?\b"></div>
          </div>
          ...
          <div class="row mb-3">
            <div class="col-sm mb-3">
              ...
              <div class="input-group w-50">
                <input type="text" class="form-control" id="month" name="month" placeholder="12" pattern="^(0[1-9]|1[012])$" required></div>
            </div>

            <div class="col-sm">
              <label for="year" class="form-label"><fmt:message bundle="${loc}" key="language.year"/></label>
              <div class="input input-group w-50">
                <input type="text" class="form-control" id="year" name="year" placeholder="2022" pattern="^202[2-9]{1}$" required></div>
            </div>
          </div>
