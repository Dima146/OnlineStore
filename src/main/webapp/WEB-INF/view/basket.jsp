        ...
        <c:if test="${sessionScope.user != null}">
            <div class="my-sm-3">
                <div class="justify-content-center">
                    <c:if test="${ordersFromUser.size() <= 0}">
                    ...
                    <table class="table table-hover">
                        ...
                        <c:forEach var="order" items="${ordersFromUser}">
                                <tr>
                                    <td>
                                        <c:forEach var="product" items="${products}">
                                            <c:if test="${product.id == order.productId}">
                                                <img src="static/images/${product.productPhotoReference}" class="img-thumbnail" width="80" height="80">
                                            </c:if>
                                        </c:forEach>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <c:out value="${order.purchaseQuantity}"/>
                                    </td>
                                    <td>
                                        <c:forEach var="product" items="${products}">
                                            <c:if test="${product.id == order.productId}">
                                                <p><c:out value="${product.productPrice}"/><fmt:message bundle="${info}" key="information.price"/></p>
                                            </c:if>
                                        </c:forEach>
                                    </td>
                                    <td>
                                        <button onclick="location.href = '${pageContext.request.contextPath}/OnlineStore?command=deleteOrderFromUser&orderFromUserId=${order.id}'"
                    ...
                    <c:if test="${ordersFromUser.size() > 0}">
                        ...
                        <h4 class="text-primary"><fmt:message bundle="${loc}" key="language.toPay"/>:
                            <c:out value="${totalCost}"/> <fmt:message bundle="${info}" key="information.price"/></h4>
                    </c:if>
                </div>
                <c:if test="${ordersFromUser.size() > 0}">
                    <div class="row justify-content-center">
                        <input type="button" class="btn btn-primary" value="<fmt:message bundle="${loc}" key="language.arrange"/>"
